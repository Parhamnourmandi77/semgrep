...
let rec check_tainted_lval env (lval : IL.lval) :
    Taints.t * S.shape * Lval_env.t =
  let new_taints, lval_in_env, lval_shape, lval_env =
...
and check_tainted_lval_aux env (lval : IL.lval) :
  ...
  | _ :: _ as sanitizer_pms ->
      ...
      in
      (Taints.empty, `Sanitized, S.Bot, lval_env)
  | [] ->
       ...
       *)
      let sub_new_taints, sub_in_env, _shape, lval_env =
        match lval with
        | { base; rev_offset = [] } ->
            (* Base case, no offset. *)
      ...
      in
      report_results { env with lval_env } results;
      (new_taints, lval_in_env, lval_shape, lval_env)
      ...
      let subst_in_precondition taint =
        let subst taints =
          ...
          |> List.concat_map (fun t ->
                 ...
                 | Src _ -> [ t ]
                 | Var lval ->
                     let+ arg_taints, _TODOshape = lval_to_taints lval in
      ...
      let process_sig :
        ...
        | T.ToReturn (taints, _return_tok) ->
            ...
            |> List_.map_filter (fun t ->
                   ...
                   | Var lval ->
                       let* lval_taints, _TODOshape = lval_to_taints lval in
...
let check_tainted_instr env instr : Taints.t * S.shape * Lval_env.t =
  ...
  let check_instr = function
    ...
    | Call (_, e, args) ->
         ...
         *)
        Logs.debug (fun m ->
            m ~tags:debug "check_orig_if_sink COMPAT %s + %s"
              (T.show_taints
                 (Taints.union all_args_taints all_args_taints_in_shapes))
              (S.show_shape shapeTODO));
